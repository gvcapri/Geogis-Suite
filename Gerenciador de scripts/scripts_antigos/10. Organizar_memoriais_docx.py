...
CAMINHO_ARQUIVO = r"João\Códigos Python\MD_LOTES_INTEGRAL_att.docx"

# Opcional: deixe vazio para criar automaticamente *_organizado.docx
CAMINHO_SAIDA = r"MD_Lotes_Vila_Operária_ATT.docx"
...
def main() -> None:
    input_path = Path(CAMINHO_ARQUIVO).expanduser()
    if not input_path.exists():
        raise FileNotFoundError(f"Arquivo nao encontrado: {input_path.resolve()}")

    if CAMINHO_SAIDA.strip():
        output_path = Path(CAMINHO_SAIDA).expanduser()
    else:
        output_path = input_path.with_name(f"{input_path.stem}_organizado{input_path.suffix}")

    total = sort_docx(input_path, output_path)
    print("CONCLUIDO!")
    print(f"Memoriais organizados: {total}")
    print(f"Arquivo criado em: {output_path.resolve()}")
