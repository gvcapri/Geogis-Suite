...
# Carrega os arquivos
df1 = pd.read_excel(r"c:\Users\Rafael Honorato\Downloads\sayonara\met_sayonara.xlsx")  #<- Aqui vai ECOLETA
df2 = pd.read_excel(r"c:\Users\Rafael Honorato\Downloads\sayonara\shp_sayonara.xlsx")     #<- Aqui vai MET
nome = "Morada do Sol - MET_ECOLETA - Comparativo.xlsx"
...
#duplicatas_MET = df2_limpo[df2_limpo.duplicated(subset=["quadra", "lote"], keep=False)]

# Cria conjuntos únicos
set_ecoleta = set(zip(df1_limpo["quadra"], df1_limpo["lote"]))
set_MET = set(zip(df2_limpo["quadra"], df2_limpo["lote"]))
...
for quadra_lote in sorted(set_ecoleta - set_MET):
...
for quadra_lote in sorted(set_MET- set_ecoleta):
    resultado.append({"Quadra": quadra_lote[0], "Lote": quadra_lote[1], "Status": "--", "Origem": "MÉTRICA"})
